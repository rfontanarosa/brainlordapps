function seventhSagaTextClean(text) {
...
function seventhSagaPreviewBox(previewContainerSelector, text, boxIndex, boxType) {
    ...
    dialogs.forEach((dialog, index) => {
        ...
        dialog = seventhSagaTextClean(dialog);
        ...
        for (let i = 0; i < dialog.length; i++) {
            ...
            if (hashcharlist[utf16char] > 0) {
                ...
                buffer = `<div class="seventhsaga-font1 seventhsaga-font1-${utf16int}"></div>`;
            } else if (utf16char == "\n") {
                buffer = '<br />';
                indexLine++;
...
function renderPreview(previewContainerSelector, text, id, type) {
    ...
    const boxType = 1;
    seventhSagaPreviewBox(previewContainerSelector, text, id, boxType);
}
